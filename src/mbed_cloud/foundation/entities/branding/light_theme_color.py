"""
.. warning::
    LightThemeColor should not be imported directly from this module as the
    organisation may change in the future, please use the :mod:`mbed_cloud.foundation` module to import entities.

Foundation Entity: LightThemeColor
==================================
...
How to import LightThemeColor directly:
...
class LightThemeColor(Entity):
    """Represents the `LightThemeColor` entity in Pelion Device Management"""
    ...
    def __init__(self, _client=None, color=None, reference=None, updated_at=None):
        ...
        self._reference = fields.StringField(value=reference, enum=enums.LightThemeColorReferenceEnum)
    ...
    def list(self, filter=None, order=None, max_results=None, page_size=None, include=None):
        ...
        `REST API Documentation <https://os.mbed.com/search/?q=Service+API+References+/v3/branding-colors/light>`_.
        ...
        :rtype: mbed_cloud.pagination.PaginatedResponse(LightThemeColor)
        """
        ...
        if isinstance(filter, dict):
            filter = ApiFilter(filter_definition=filter, field_renames=LightThemeColor._renames_to_api)
        # The preferred method is an ApiFilter instance as this should be easier to use.
        elif isinstance(filter, ApiFilter):
            # If filter renames have not be defined then configure the ApiFilter so that any renames
            # performed by the SDK are reversed when the query parameters are created.
            if filter.field_renames is None:
                filter.field_renames = LightThemeColor._renames_to_api
        elif filter is not None:
            raise TypeError("The 'filter' parameter may be either 'dict' or 'ApiFilter'.")

        return paginate(
            self=self,
            foreign_key=LightThemeColor,
            filter=filter,
            order=order,
