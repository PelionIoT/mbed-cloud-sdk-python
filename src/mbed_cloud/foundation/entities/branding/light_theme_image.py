...
class LightThemeImage(Entity):
    ...
    _fieldnames = ["reference", "static_uri", "updated_at"]
    ...
    def __init__(self, _client=None, reference=None, static_uri=None, updated_at=None):
        ...
        self._reference = fields.StringField(
            value=reference, enum=enums.LightThemeImageReferenceEnum
        )
    ...
    def list(self, filter=None, order=None, max_results=None, page_size=None, include=None):
        ...
        if isinstance(filter, dict):
            filter = ApiFilter(
                filter_definition=filter, field_renames=DarkThemeImage._renames_to_api
            )
