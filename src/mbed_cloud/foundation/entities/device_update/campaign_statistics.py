...
class CampaignStatistics(Entity):
    ...
    _fieldnames = ["campaign_id", "count", "created_at", "id", "summary_status"]
    ...
    def __init__(
        ...
        self._summary_status = fields.StringField(
            value=summary_status, enum=enums.CampaignStatisticsSummaryStatusEnum
        )
    ...
    def events(
        ...
        if isinstance(filter, dict):
            filter = ApiFilter(
                ...
                field_renames=CampaignStatisticsEvents._renames_to_api,
            )
    ...
    def list(self, filter=None, order=None, max_results=None, page_size=None, include=None):
        ...
        if isinstance(filter, dict):
            filter = ApiFilter(
                filter_definition=filter, field_renames=CampaignStatistics._renames_to_api
            )
        # The preferred method is an ApiFilter instance as this should be easier to use.
        elif isinstance(filter, ApiFilter):
            # If filter renames have not be defined then configure the ApiFilter so that any renames
    ...
    def read(self):
        ...
        return self._client.call_api(
            ...
            path_params={
                ...
                "summary_status_id": self._id.to_api(),
            },
            unpack=self,
        )
