...
class DefaultApi(object):
    ...
    def update_campaign_stop_0(self, campaign_id, **kwargs):
        """
        Stop a running update campaign.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async=True
        >>> thread = api.update_campaign_stop_0(campaign_id, async=True)
        >>> result = thread.get()
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async'):
            return self.update_campaign_stop_0_with_http_info(campaign_id, **kwargs)
        else:
            (data) = self.update_campaign_stop_0_with_http_info(campaign_id, **kwargs)
            return data

    def update_campaign_stop_0_with_http_info(self, campaign_id, **kwargs):
        """
        Stop a running update campaign.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async=True
        >>> thread = api.update_campaign_stop_0_with_http_info(campaign_id, async=True)
        >>> result = thread.get()
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method update_campaign_stop_0" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'campaign_id' is set
        if ('campaign_id' not in params) or (params['campaign_id'] is None):
            raise ValueError("Missing the required parameter `campaign_id` when calling `update_campaign_stop_0`")
