...
def _main():
    ...
    print("%s\n%s" % (header, len(header) * "-"))
    for i, metric in api.get_metric(None, None, None, "30d", "1d").iteritems():
        print metric

    header = "Get statistics from the last 2 days in 3 hours interval"
    print("%s\n%s" % (header, len(header) * "-"))
    for i, metric in api.get_metric(None, None, None, "2d", "3h").iteritems():
        print metric

    header = "Get statistics from 1 March 2017 to 1 April 2017"
    print("%s\n%s" % (header, len(header) * "-"))
    for i, metric in api.get_metric(None, start, end, None, "1d").iteritems():
        print metric
